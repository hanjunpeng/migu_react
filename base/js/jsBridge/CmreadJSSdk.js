...
~function(window){
	...
	window.jsbCallback = {
		add: function(ch, callback){
			listeners[ch] ? listeners[ch].push(callback) : listeners[ch] = [callback];
		},
		call: function(ch, key1,key2,key3){
			if(!(ch in listeners))
				return;
			var arr = listeners[ch].slice();
			~function next(){
				var callback;
	...
	window.setClientValue = function(key1,key2,key3){
		var key1 = key1?key1:null,
		key2 = key2?key2:null,
		key3 = key3?key3:null;
		jsbCallback.call("setClientValue",key1,key2,key3);
	}
